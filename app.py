...
app.config['MYSQL_PASSWORD'] = 'Aeroplane135#'
...
def is_logged_in(f):
    ...
    def wrap(*args, **kwargs):
        if 'logged_in' in session:
            ...
            flash("Unauthorized. You will be redirected to the login page.", "danger")
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if isnewuser(username):
            ...
            flash('This user already exists. Please try another username, or login.', 'danger')
    ...
    if request.method == 'POST':
        ...
        if accPass is None:
            ...
            if sha256_crypt.verify(candidate, accPass):
                log_in_user(username)
                flash('You are now logged in. You will be directed to the dashboard', 'success')
                return redirect(url_for('dashboard'))
            else:
...
def transaction():
    ...
    if request.method == 'POST':
    	try:
            ...
            flash(str(e), 'danger');return redirect(url_for('transaction'))
...
def logout():
    session.clear()
    flash("Logout success. You will be redirected to the login page.", "success")
    return redirect(url_for('login'))
